...
var sky,backgroundImg, invisibleGround
...
if(rocket.isTouching(topGroup)||rocket.isTouching(bottomGroup) || rocket.isTouching(invisibleGround)){
  gameState = end;
...
function topPipe(){
  if(frameCount % 150 === 0){
    obstacle = createSprite(displayWidth,random(50,200));
...
function bottomPipe(){
  if(frameCount % 175 === 0){
    bObstacle = createSprite(displayWidth,random(600, 800));
    bObstacle.velocityX = -4
    bObstacle.addImage(pipe);
    bObstacle.scale = 0.8
    bObstacle.lifetime = displayWidth/4;
bObstacle.debug  = false;
    bottomGroup.add(bObstacle);

    bObstacle.setCollider("rectangle",-50,150,150,500)
